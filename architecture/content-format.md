...
Coverage is partial and the gap is uneven. Brief mode already emits `built_from`; record mode binds by `record_hash`; **node mode, the majority of the corpus, carries no binding at all**. Node mode can carry claim hashes as soon as the assembler's query selects the `claim_hash` column that already exists in `knowledge.db`.
