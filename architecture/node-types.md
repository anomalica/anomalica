...
>
> This amendment governs the node NAME - the graph identity, the slug, and the matcher's input. It does not govern the rendered page title. Since 2026-09-03 the page title writes exactly two acronyms bare, UFO and UAP, with no expansion, pluralised when they stand alone (the title rule in [editorial-style.md](../guides/editorial-style.md), Acronyms). The node stays "Unidentified Flying Object (UFO)" and its slug stays `unidentified-flying-object-ufo`; only the headline the reader sees becomes "UFOs" (plural where the acronym is the head of the title, per the same rule). The two rules are not in conflict: one names the thing in the graph, the other renders it on a page.
...
The rule: do not create a person node for a redacted or anonymous actor. Where the anonymous actor is somebody a document DESCRIBES, attribute the claim to the relevant organisation (3rd Fleet Intelligence, USS Louisville, Strike Fighter Squadron 41), and describe the role in the claim text ("a 3rd Fleet intelligence officer stated..."). If a later document identifies the same actor by name, that document produces the person node and the earlier claim can be re-attributed during review.

**That reassignment does not apply to somebody SPEAKING** - see the next paragraph. An organisation is the right home for a claim about an unnamed officer's actions; it is the wrong home for an unnamed person's own testimony, because attributing it to a body asserts that the body said what the person said. An `[audience member]` frequently has no organisation to reassign to at all.

**An anonymous SPEAKER is the same rule, said in the record itself.** A transcript writes a speaker whose real name is unknown as a description in square brackets - `[interviewer 2]`, `[audience member]`, `[recovery team member]`, `[speaker 3]` - and the brackets mean exactly what this section means: not a person. They are scoped to their own record, so the `[interviewer 2]` in one recording is not the one in another. Never create a person node from a bracketed speaker, never merge two same-spelled ones, and never accumulate a biography under one. What they said still becomes a claim, quoted and attributed to the description as written; it is the identity that does not exist, not the testimony. Older records write the same thing unbracketed as `Speaker 1` - treat both alike. The notation is specified in [ingest-format.md](ingest-format.md#square-brackets-mean-this-is-a-description-not-a-name).

The same notation is the right way to write an anonymous actor a source only DESCRIBES, where there is no organisation to attribute to and the claim would otherwise lose who said it: `[Anomaly Physical Evidence Group (APEG) biochemist]`, not a person node named `unnamed Anomaly Physical Evidence Group (APEG) biochemist`. The brackets are the machine-readable half of the rule - a consumer can tell a description from a name without guessing at a lower-case "unnamed" prefix.
...
- **Aliases, and the retained surname-first form.** Informal names, shortened forms, and transliterations are aliases, not the canonical name (canonical "Luis Elizondo"; aliases "Lue Elizondo"). The old surname-first order ("Elizondo, Luis") is ALSO retained as an alias - deliberately: it carries migration (pre-re-digest digests are last-first) and preserves the comma structure the importer's matching precision keys on. The importer accepts last-first INPUT and normalises it to the natural canonical; the matcher stays order-tolerant permanently (real names arrive both ways), not dropped after re-digestion.
  ...
  **These aliases are load-bearing, and a tidy-up would look reasonable.** Measured 2026-08-22: 245 distinct `provenance_chain.origin` values across 704 claims are still written surname-first, and every one of them resolves ONLY because the surname-first string is in the alias index. Delete the aliases as redundant - which is what they look like once every node name is natural order - and those 704 claims lose their provenance root silently: no error, no empty column, just an origin that stops resolving to a node and falls back to being counted as its own root. That inflates independent-source counts, which the page gate reads. The aliases are not a migration artefact to be cleaned up after the corpus settles; they are how last-first text still resolves, and last-first text keeps arriving.
