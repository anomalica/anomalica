...
The image annotation is split at the pre-digest ([decision 0042](../decisions/0042-pre-digest-stage-and-eval-only-highlights.md)). Its `description` is CONTENT and is **KEPT** - rendered as prose to the model, so a claim can be extracted from what the image actually shows (the text of a screenshotted tweet, a chart's figures). Everything else on the annotation - `file`, `alt`, `caption`, and the `irrelevant` flag - is metadata or display and is **stripped before extraction**. The whole annotation stays in the record for consumers that render the image (the assembler); the pre-digest (model input) drops only the metadata fields.

**Both annotation forms carry the description.** In the mapping form it is the `description:` field. In the single-line inline form (`<!-- image: Grainy photo of an oblong object... -->`) the scalar value IS the description - there is no `alt` or `caption` on the inline form to confuse it with - so an inline image annotation is kept and extractable in full, exactly like a mapping's `description`.

The `caption` versus `description` split is the crux: a caption is the source's own attribution and often carries a copyright notice, which must never become a factual claim, so it is stripped; a description is a faithful transcription of the image's content, which is exactly what the model should be able to extract, so it is kept.
...
Here `description` (the tweet's transcribed text) reaches the model and can become a claim; `caption` (the attribution) does not.

A reviewer's keep/drop choice for an image is the `irrelevant` flag on the same annotation (`irrelevant: true`; absent means keep), mirroring the text [mark-irrelevant convention](review-workbench.md#what-to-mark-irrelevant). The flag is display metadata, not content, so the pre-digest drops it and it never touches extraction - it is purely a DISPLAY decision, read by the assembler/site to choose which images to render. Like other reviewer corrections, the who/when rides on the git commit that sets it, not an inline stamp:
