...
| `source_hash` | string | no | Emitted only by the `web` handler: SHA-256 of the raw HTML asset, locating `sources/{source_hash}.html`; differs from `content_hash` (which for web hashes the extracted body). No other type emits it: `audio`/`video`/`pdf` use `content_hash` (which hashes the source bytes) as the source-asset hash at `sources/{content_hash}.{ext}`; `ebook` writes no source-bytes hash in frontmatter and locates its source asset only via the verification sidecar's `sha256` and the `sources/{hash}.{ext}` filename. |
