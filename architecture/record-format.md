...
Image annotations - the whole mapping, including `caption` and `description` - are **stripped by the pre-digest before extraction** ([decision 0042](../decisions/0042-pre-digest-stage-and-eval-only-highlights.md)): they are image metadata, not prose the model reads for claims. The annotation stays in the record for consumers that render the image (the assembler); only the model input drops it. This matters most for the caption: a source's printed caption often carries a copyright or attribution notice, and a copyright notice must never be extracted as a factual claim. Structuring the caption ON the image - rather than leaving it as loose prose beneath the figure - is what lets the pre-digest strip it cleanly.

A reviewer's keep/drop choice for an image is the `irrelevant` flag on the same annotation (`irrelevant: true`; absent means keep), mirroring the text [mark-irrelevant convention](review-workbench.md#what-to-mark-irrelevant). Because every image annotation is already stripped from the pre-digest, this flag never touches extraction - it is purely a DISPLAY decision, read by the assembler/site to choose which images to render. Like other reviewer corrections, the who/when rides on the git commit that sets it, not an inline stamp:
