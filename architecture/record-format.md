...
Non-reuse is enforced across edits by persisting the id high-water mark in frontmatter, `overlay_next_id` (the next id to mint). Without it the guarantee would be a fiction: a counter held only in memory resets on reload, and the workbench derives its high-water from the ids *currently* in the body - so deleting the highest-numbered marker would lower the mark and reissue that id. Persisting it means reloading a record, or deleting its highest marker, can never lower it. The field is lazily migratable: when absent, derive the high-water from the largest id extant in the body - correct for every record that has never deleted its highest. Overlay ids share **one id space per record** across highlights, span notes, and links, so a single counter keeps every overlay id unambiguous and every reference (in-record and the external `record-hash + id` link address) safe.
