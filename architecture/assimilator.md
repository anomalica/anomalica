...
### Relate (EXPERIMENTAL, 2026-09-03)

Does one record refer to the same specific subject as another - one incident, operation, programme, document or investigation - under different words? Nothing else in the pipeline answers that: shared entity nodes are corpus-wide hubs, claim similarity asks "same fact?", and records rarely cite each other. `assimilator relate` shortlists record pairs by claim neighbourhood (each claim's twenty nearest claims in other records, hits counted per record, the top fifteen records per record) and has a model read both claim lists with a strict prompt that must quote a pinning particular from each side; verdicts (`same_subject`, `possibly_related`, `unrelated`, the shared subject, the reason, the linking claim pairs) go to the derived `record_relations` table. Unrelated verdicts are stored too, so a pair is judged once. The model resolves through the policy's `relate` stage. Nothing links or merges on a verdict; a reviewer confirms in the workbench later. Not part of rebuild or the scheduler. Evidence: `assimilator/reports/cross-source-linking-2026-09-02/`.
