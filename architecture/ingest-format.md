...
**Never expand, normalise or substitute inside a person's name.** A glossary, an acronym expander, or any term-normalisation pass says what a term means in prose; a person's name is not a term. `UAP Gerb` - a real researcher, 36 claims and 8 independent sources, page-worthy - was silently rewritten to `Unidentified Aerial Phenomena (UAP) Gerb` by a per-document glossary expanding the acronym inside his name. The corruption was hard to spot precisely because the expanded form looked MORE authoritative than the original, not less: a name that has been "helpfully" completed reads as the careful version. Exempt name fields from every such pass.
...
- **Inert to MATCHING, not merely to minting.** A description must never resolve to an existing node, and the guard belongs at the matcher rather than at each call site. Guarding only node creation leaves every other resolution path open, and a fuzzy tier will happily score `[Anomaly Physical Evidence Group (APEG) biochemist]` against the node `unnamed Anomaly Physical Evidence Group (APEG) biochemist` it was written to replace - the bracket and one word being the only difference. The notation then silently reintroduces the exact node it exists to prevent. One rule at the choke point covers references, speakers and minting together.
- **An anonymous producer keeps its description.** Where a record's author or producer is a description, it resolves to no node - correctly, since no such person exists - but the record must still carry the description as written rather than being left with an empty producer. Who produced a document is a property of the document, and "anonymous senior intelligence officer" is materially different from "no author recorded": the first is a document whose author was deliberately withheld, the second is a gap in our own metadata. Losing that distinction costs a real evidence signal, because an anonymously-authored source carries different weight from an attributed one.
...
**Where a handle is borderline, ask whether another record would use it to mean this same person.** `A Friend` is a source pseudonym in the literature, so a second record discussing that case uses it for that man - it identifies him, which is what makes a pseudonym a name. `"the boy"` does not: it is a referring expression that means whoever the surrounding record was talking about, and no other record's "the boy" is him. A handle that cannot carry identity out of its own record is a description wearing quotation marks.

The test deliberately ignores form, because form cannot carry it. `Sally (Budd Hopkins abductee)` and `Senior U.S. Intelligence Officer (Anonymous)` are structurally identical and land on opposite verdicts, so any rule keyed on the parenthetical, on the word "Anonymous", or on capitalisation gets one of them wrong. It also covers the speaker case without a second rule: a `<!-- speaker: -->` value that is entirely a description names nobody by the same test.
...
**The bracket is the authority; a sweep is only a review aid.** A query keyed on a lower-case `unnamed` prefix misses `Senior U.S. Intelligence Officer (Anonymous)` and every other capitalised description - the leading word is a notation and notations vary. But testing name *shape* instead is worse, not better: a shape test written against this rule flagged `Colonel Friend` (a real surname) and still missed `Senior U.S. Intelligence Officer`, because "U.S." splits into capitalised tokens that read as a proper noun. Inferring intent from a stored name after the fact is unreliable in both directions.

What is reliable is the marker written at the moment the value was minted, when the source was in front of the model that read it. That is the whole reason the notation exists. Sweep to find candidates for a human to look at; never let a regex conclude that a set is complete, and never let one silently reclassify a name.

A guard taught by worked examples fails the same way a regex does: a nodes prompt carrying the category plus one example phrasing (`USS Louisville Officer (redacted)`) still minted `Senior U.S. Intelligence Officer (Anonymous)` as a person, because a model shown an example matches the example rather than applying the category.
