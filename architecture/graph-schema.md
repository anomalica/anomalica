...
| `records` | `id` PK, `title` NOT NULL, `reference`, `date`, `producer_id`, `content_hash`, `friendly_name`, `metadata` (JSON text), `created_at` NOT NULL, `work_id` | One row per source record. `work_id` groups multiple record manifestations of the same work for independent-source counting; it defaults to the record's own `id` until deterministic duplicate-work linking assigns a shared ID. |
...
- **`records.work_id`** is derived, rebuildable identity rather than a new source
  assertion. Page gating and source-spread selection count distinct
  `COALESCE(work_id, records.id)` values so an unlinked record remains one source
  and two downloads, exports or editions of one linked work cannot satisfy a
  multiple-work threshold. Corroboration confidence separately counts distinct
  provenance-chain roots rather than record or work rows.
