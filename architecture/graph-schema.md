...
- **`records.work_id`** is derived, rebuildable identity rather than a new source
  assertion. Consumers count distinct `COALESCE(work_id, records.id)` values so
  an unlinked record remains one source and two downloads, exports or editions of
  one linked work cannot satisfy a multiple-independent-source threshold.
