...
- **AI-spend transparency** - AI processing runs on paid Anthropic services attributable to the project, with a hard cost gate before any metered run. The transport (a Claude subscription by default, or the metered API behind a per-component toggle) and the billing detail are tracked privately in `operations`; enforcement lives in the inherited root CLAUDE.md.
