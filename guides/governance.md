...
- **AI-spend transparency** - AI processing runs on paid Anthropic services, with a hard cost gate before any metered run. Metered-API spend is real per-project dollars and is published; the default Claude subscription is founder-funded at a flat rate, not separately metered per project. Which transport each component uses, and the billing detail, are tracked privately in `operations`; enforcement lives in the inherited root CLAUDE.md.
