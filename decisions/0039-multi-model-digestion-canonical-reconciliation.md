...
- **Two stores.** The digester emits **raw digests** (1..N per source - several models, or re-digestions over time); the fuser produces **fused digests**. Only the fused digests are assimilated; the raw digests are the rebuildable inputs. Pipeline: digester -> raw digests -> fuser -> fused digests -> assimilator.
- **Fusion is at the claim level.** Detect equivalent claims across the versions and collapse them into one canonical claim - merging, not selecting one variant whole -> fused claims. The unit of fusion (and of review) is the claim, not the whole digest.
- **Review influences the fuser; it does not edit digests.** Editing a derived artefact by hand is the risk, so reviewers don't touch digests directly. They review the fuser's claim choices and express preferences ("this variant's claim is better"), which are a **replayable record the automated fuser replays** - the same pattern as graph curation ([0038](0038-graph-curation-replayable-ledger.md)). The fused output stays rebuildable = automated fusion + replayed human preferences; a changed preference re-fuses, and the supersession diff falls out.

Architecture diagram source (the fuser node): `reference/pipeline.mmd` + `reference/architecture.yaml` in the meta-repo.
