# 0037. An AI-operation ledger: cost and provenance per call
...
Every AI or compute call the pipeline makes - a digester extraction, an assimilator consolidation, an assembler write, an ingester transcription - costs something (metered dollars, subscription rate-limit budget, or GPU/CPU time) and is a provenance fact (which model, which version, produced this output). Today none of it is persisted. Token usage exists only as a process-global in-memory accumulator in the shared transport (`anomalica-common`'s `llm` module - per-run, lost on process exit) and as a transient `USAGE_JSON:` stdout line the workbench runner scrapes into a capped 50-entry `processing.json`. There is no cross-component record, no per-call granularity, no model-version provenance, and no persistent cost history.

Two needs converge on one store:

- **Cost.** [Governance](../guides/governance.md) commits to publishing metered per-project spend and funding subscription and local compute at a flat, founder-funded rate. That published figure needs a data source.
...
- `reported_cost_usd` - the cost the provider reported for this call. The subscription CLI returns `total_cost_usd` (the would-be dollar value of the call); the metered API reports none (null), as do the GPU/CPU paths. This shows what a subscription call would have cost without implying money changed hands.
- `billed_usd` - dollars actually charged to a metered Anomalica account. Metered API: tokens x `MODEL_PRICING` (the API path reports no cost, so the writer computes it from the rate table). Subscription, GPU, CPU: 0 (founder-funded flat-rate / local compute). Summed per component, this is the published metered figure governance commits to.
...
- **One store, both jobs.** Cost accounting and model provenance are per-call facts about the same event; splitting them would double the instrumentation.
...
A new operational store, a shared writer, and per-site emits. It realises the governance AI-spend principle (the published metered figure) and adds model provenance to the [0008](0008-content-traceable-to-sources.md)/[0010](0010-auditable-assembly.md) audit trail. It changes no model behaviour. The operation and transport enums align to the scheduler's existing job vocabulary. Field detail is in [architecture/ai-ledger-format.md](../architecture/ai-ledger-format.md) (`anomalica/ai-ledger/1`), scaffolded now and filled as the writer lands at each site.
