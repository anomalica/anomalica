...
After assembly, a different AI model from a different provider (decision 0009) independently verifies that every factual assertion in the article traces to the knowledge graph. The verification report is stored alongside the article, including which model performed the check, when, and what it found.
