# 0009. Transparent use of artificial intelligence with independent verification
...
Content assembled by artificial intelligence (AI) carries a reputation problem. Many people associate AI-produced text with hallucination, bias, and a lack of accountability. For a platform whose credibility depends on accuracy and traceability, using AI to assemble articles could undermine trust if not handled openly.

At the same time, the platform's design requires AI. Maintaining thousands of articles across 30 languages, keeping them current as the knowledge graph (a structured database of interconnected facts) evolves, and ensuring every assertion traces back to a specific source - this is not feasible with manual editing alone. AI is not a shortcut; it is the mechanism that makes the platform's core promises possible. The distinction between AI that creates content and AI that assembles content from existing sources is addressed in decision 0008.
...
- **Why AI is used** - the scale of maintaining articles across 30 languages with full source traceability requires automated assembly. AI enables provenance chains (the path a claim took from its original source to the knowledge graph) that would be impractical to maintain manually.
...
Some users will distrust AI-involved content regardless of the safeguards. AI is involved at every stage - extracting claims from source documents, building the knowledge graph, assembling articles, and translating. These are unavoidable black boxes (processes whose internal workings are not fully visible). What the platform can do is make the inputs and outputs of each black box visible: here is the source document, here is the digest (what was extracted from it), here is the assembled article, here is the verification report. A reader can check the source document themselves and judge whether the digest is accurate. The aim is not to eliminate the need for trust, but to make the platform as verifiable as possible at every step.
