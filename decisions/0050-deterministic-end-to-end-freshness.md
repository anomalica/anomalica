...
Priority is not authorisation. In particular, unknown digest extraction
generation is a scheduling signal and is not permission to invoke a model. A
full-corpus generation-1 re-digestion is not authorised by this decision, by the
generation-1 manifest, or by a scheduler finding. It requires a separately
costed and explicitly authorised batch. The same separation applies to every
metered or plan-limited regeneration.
...
unknown old payload binding and is not current; deterministic regeneration closes
that gap without pretending the old selection hash covered values it omitted.
